...
### M3 · 推理层接入
**做**：`ClaudeReasoner` 实现 §7 六方法；`route` 调它对意图层节（流程/规则/功能）判
分类/置信度/`build_status`/`nature`；低置信度 & 规范性 → `queue.yaml`；意图层正文不改写、只标 `待PM确认`。

**验证**：样例 §3 规则表「目前未实现」诸条被判出 `build_status` 并标"需对照基线代码核实"；
规范性意图进 PM 队列；"A程序"占位名进队列（架构类）。
