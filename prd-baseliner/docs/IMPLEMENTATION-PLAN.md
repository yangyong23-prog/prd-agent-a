...
### M4 · 组装出真实样例　✅ 已完成
**做**：`emit/markdown.py` 渲染成品——标记图例 → 文档级 → 各节（载重字段 + 正文逐字 + 就地标记）→ 末尾两张清单（A. 需代码·生成/核实；B. 需PM·确认/决策/本体），五类标记自动产出。

**实测**（真实 PRD）：46 节、约 106KB 成品；清单 A 含 380 端点拆 4 服务 + 3 表 + 241 枚举（生成）+ 空接口/规划中（核实），清单 B 含 A程序架构决策 + 本体对齐 + 规范性决策。安全示例（占位 fixture，无真实 PRD）见 `examples/output/示例-基线PRD章节实例.md`。测试见 `tests/test_m4.py`。
